...
def draw_detection(
    ...
    keypoint_drawing_spec: DrawingSpec = DrawingSpec(color=GREEN_COLOR),
    bbox_drawing_spec: DrawingSpec = DrawingSpec(), draw_marker=False, only_centered=False, center_margin=0.10):
  ...
  if draw_marker and rect_end_point is not None and rect_start_point is not None:
      width=rect_end_point[0]-rect_start_point[0]
      height=rect_end_point[1]-rect_start_point[1]
      marker1_start_point=(int(rect_start_point[0]+width/4),rect_start_point[1])
      marker1_end_point=(int(rect_end_point[0]-width/4),rect_end_point[1])
      marker2_start_point=(int(rect_start_point[0]+width/2),rect_start_point[1])

      h, w, c = image.shape


      if only_centered:
          marker_margin_left = marker1_start_point[0]
          marker_margin_right = w - marker1_end_point[0]
          is_centered = abs(marker_margin_right - marker_margin_left) < int(w*center_margin)
      if not only_centered or is_centered:
          marker_applied=True
          cv2.rectangle(image, marker1_start_point, marker1_end_point, GREEN_COLOR,-1)
          cv2.rectangle(image, marker2_start_point, marker1_end_point, WHITE_COLOR, -1)
